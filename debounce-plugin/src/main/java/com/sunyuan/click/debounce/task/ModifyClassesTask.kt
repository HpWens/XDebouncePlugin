...
import com.sunyuan.click.debounce.extensions.getBootClasspath
 ...
 * @author sy007
 * @date 2023/07/13
 * @description DefaultTask 是所有自定义 Gradle 任务的父类，提供了任务的基本结构和生命周期管理。
 */
abstract class ModifyClassesTask : DefaultTask() {
    ...
    fun taskAction() {
        ...
        val bootFiles = project.getBootClasspath()
    ...
    private fun transform(
        entity: ProxyClassEntity,
        canonicalName: String,
        bytes: ByteArray
    ): ByteArray = if (PathMatcherUtil.matchClassPath(canonicalName)) { // 在白名单内
